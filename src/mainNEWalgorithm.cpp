...
int distanceSensorStateWhenHIGH();
int distanceSensorStateWhenLOW();
...
void loop(){
  ...
  if(distanceSensorState == HIGH){
    distanceSensorStateWhenHIGH();
  }
  else
  {
...
int distanceSensorStateWhenHIGH(){
  do{
    distanceSensorState = digitalRead(distanceSensor);
    if(intAppButton == 0 && distanceSensorState == HIGH){  //idle
      digitalWrite(relay, HIGH);
      lcd.backlight();
      lcd.clear();
      lcd.setCursor(0,0);
    ...
    else if(intAppButton == 1 && distanceSensorState == HIGH){ //decision making if intAppButton value = 1
      ...
      Serial.print("The intAppButton value = " + String(intAppButton) + " --> Door UNLOCKED via App Button");
  ...
  }while(distanceSensorState == HIGH);
}
  ...
  do{
    ...
    do{
      if(intCamera == 0 && distanceSensorState == LOW){ //idle
        ...
        lcd.setCursor(0,1);
        lcd.print("Face Recognition");
        Serial.print("The intCamera value = " + String(intCamera) + " --> Door UNLOCKED via Face Recognition");
    ...
    }while(cameraAlgorithm);
  }while(distanceSensorState == LOW);
}
...
// void loop(){
//     distanceSensorState = digitalRead(distanceSensor);
//     if(distanceSensorState == HIGH){ //if distance sensor detects no human
//         distanceSensorStateWhenHIGH:
//         delay(200);
//         do{
//             distanceSensorState = digitalRead(distanceSensor);
...
//             }
//             else if(distanceSensorState == LOW){
//                 goto distanceSensorStateWhenLOW;
//                 delay(200);
//             }
//         }while(distanceSensorState == HIGH);
//     }
